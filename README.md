...
const styles = trousers('button').element`
...
const spanStyles = trousers('button-span').element`
        font-size: 20px;
        font-style: italic;
    `;
...
```jsx
const buttonStyles = // Base styles, these are static and every modifier will be applied on top
trousers('button').element`
...
```jsx
const buttonStyles = trousers('button').element`
...
const styles = trousers('button').element``.modifier(props => {
...
Mount one or more styles definitions as globals
